...
> (3) 知道还差什么、下一步往哪走, 无缝接上继续迭代。作者: 历任会话的 Notion AI agent。最后更新: 2026-06-28。
...
- **你(agent)不能直接 SSH 登服务器**。你和 GCP 服务器之间的唯一通道是 **GitHub**(用 GitHub MCP 工具读写)。
...
- **最新阶段(2026-06-28)**: 盘前**八张表逐字段价值评审全部完成**(jobs 0050–0074), 完整台账见根目录 `FIELD_VALUE_REVIEW.md`。
  下一步: T-1 滞后表汇总 → 独立因子清单定稿 → `compute_edge_v9` 升级到 v10。**下一个 job id = 0075。**
...
3. **切记: 对 main 的提交必须串行**(一个一个来)。并行双提交会 409 冲突。
4. 队列文件格式: `{ "id":"0075", "script":"scripts/xxx.py", "args":[], "timeout":900, "note":"人话描述" }`。`id` 必须唯一且与文件名主干一致。
   - **已用到的最大 id: `0074`。下一个用 `0075`。**
...
5. 读结果: 从 `agent-results` 读 `projects/duanxianxia/reports/_audit/agent_jobs/<id>.result.json`(`ok`/`rc`/`stdout_tail`/`stderr_tail`/`duration_s`), 以及脚本写出的报告文件(同在 agent-results)。
   - ⚠ stdout_tail 是"尾部"截断; **把最关键的输出放在脚本最后打印**, 大 dump 易被从前端截掉(0072 踩过)。
...
- 读: `connections.mcpServer_github.runTool({ toolName:"get_file_contents", toolArguments:{ owner, repo, path, ref } })`。`ref`=`refs/heads/main` 读代码/队列; `refs/heads/agent-results` 读结果。也可列目录(传目录 path)。
- 写: `create_or_update_file({ owner, repo, branch:"main", path, message, content[, sha] })`。新建不传 `sha`; 改已有要先读 `sha`。
- 找文件: `search_code` 只索引默认分支(main); 非默认分支的文件搜不到, 要用列目录/`get_commit` 定位。
...
- **v10_amt edge** (生产中, `duanxianxia_v9_edge.py::compute_edge_v9`):
  ...
  ```
  - `amt_pct` = 竞价成交额当日横截面百分位(0–100)。权重可被 params 覆盖。
...
- `scripts/v10_optimize.py` — 主优化管线 + 复用助手: `Daily(PROJECT_ROOT).excess(code,date)`, `spearman(xs,ys)`(对数少/零方差返回 None), `daily_ic`, `mean_icir`, `_norm`, `code_of`, 常量 `CORE_FIELDS`/`RANK_FIELDS`。
...
- **final_score 谜题定论**: 全链路从不算 final_score, 决策只走 edge_score。小缺口: 早期交易日存盘 edge 用的是旧公式, 需全史重跑 v9 统一存量 edge。
...
| ⑦ | 热度榜 rank.hot_stock_day + 飙升榜 rank.rocket | **09:25** | 见下"多轮验证"; 名次❌, 价值在两榜交叉 |
| ⑧ | 情绪指标盘 home.qxlive.top_metrics | 09:28 | ✅ **首个市场级逆向择时**: QX/ZT/ZTBX/LBBX/LBGD timing IC -0.6~-0.74 |

#### 表⑦ 多轮验证发现(本轮重点, 含一次自我纠正)
- **命名更正(0073 官方 dataset_label 铁证)**: `rank.hot_stock_day`=**热度榜(日)**=hot_stock_day, 性质 **绝对热度 LEVEL**(千万级元); `rank.rocket`=**飙升榜**=skyrocket_hour, 性质 **小时增量 DELTA/动量**(带符号万级)。二者均 **09:25 真盘前**抓取。
  > ⚠ 曾一度把它们错认成 `pool.hot/pool.surge`——后者其实是 **10:02 盘中**的"热门/冲涨"(getFxPoolData/getCzPoolData), 不同的榜, 只能 T-1 滞后用。新对话勿再混淆。
- **名次无效(0066+0074)**: 两榜 rank/raw_rate 的线性 IC 与 per-day spearman 全 ≈0 -> ❌ 不作选股排序, 只保留"是否上榜"布尔 + 交叉关系。
- **飙升 top10 肥尾(0068)**: 看似 +1.794 实为 2 异常日驱动, 胜率 41%, 不可交易。
- **regime(0070)**: 正相关, 情绪 HOT 日才 work; ×抢筹显著增强。
- **level×delta 交叉(0074, edge 真正所在)**:
  - C 热度top10 且非飙升(滞涨主线大票): ICIR 0.385 / 胜率 71% -> ✅ **稳健底仓**
  - B 飙升top10 且非热度top20(新晋边际资金): 均值 +4.1 但胜率 44% -> ⚠ **进攻彩票仓**(×抢筹优先, 须仓控)
  - A 两榜霸榜龙头: -0.69 / 胜率 35% -> ❌ **回避**(拥挤透支)

#### 跨全表"有价值字段"汇总(给 v10 用)
- 🟢 独立选股 alpha: 竞价成交额万(0.163)、竞价换手率(0.135)、竞价主力净额/成交额(0.120)、gap。**组合 comp_SD{成交额+换手率+gap}去相关 IC 0.179/ICIR 0.93 = 目前最强可交易信号**。
...
- ❌ 明确废弃: 各种榜的 rank/raw_rate/value、grab_strength、量比倍数、拆单字段、kaipan 板块全字段、HSLN/KQXY/PB 死字段(恒0/占位符)、飙升 top10 肥尾彩票。

#### Job 台账(本轮 0050–0074)
0050-0056 干净IC/封单/小里程碑; 0057 表②; 0058-59 表③; 0060 表④; 0061-63 失败; 0064 表⑤; 0065 表⑥; 0066 表⑦线性IC≈0; 0067 失败; 0068 飙升肥尾复核; 0069 表⑧情绪择时; 0070 热度/飙升 regime; 0071 pool探针(无文件); 0072 全字段dump; 0073 dataset_label定名+时点; 0074 level×delta交叉。
...
### 4.1 盘前模型 (主线, 按此顺序推进)
**逐表字段评审已全部完成。下一阶段三步:**
1. **T-1 滞后表汇总定稿**: 把盘后表(review.fupan.plate / home.ztpool / cashflow.* / review.*)的滞后价值系统过一遍, 确认除"昨日成交额(IC0.103)"外是否还有可用滞后字段。
2. **独立因子清单定稿**: 把全表评审结论收敛成两层清单 —— (a) 选股 alpha 层(comp_SD 主因子 + 委买主力净额 + 各条件过滤); (b) 市场级择时 overlay 层(qxlive 情绪逆向)。明确每个因子的方向、cohort、权重量级。
3. **重构 `compute_edge_v9` -> v10**: 把交叉/条件信号与择时 overlay 并进来。**每次改公式必须走 walk-forward 出样本验证**, 否则不上线。

**其它待完善:**
- 生产小缺口: 触发一次全历史 v9 重算, 让存量 `edge_score` 统一到 v10 公式(v10_optimize 的 regen_missing 只补缺不覆盖, 全重算需单写作业)。
- 验证路线图: 条件/交互 IC -> archetype 分组 -> walk-forward -> 受约束的 torch LTR(v22)。
- 表⑧情绪择时 n=11 样本薄, 待更多交易日积累后再确认幅度并锁定硬规则。
- 热度/飙升交叉的 B/C 组、飙升×抢筹(n=3)样本偏小, 需更多日加固。
...
## 5. 用户偏好 / 踩过的坑
- 用户要**结果**, **不要反复问确认**("不要反复问我确认, 直接给结果"), 不要"什么都靠试"——先看明白源码/页面再动手。
- "随时记录前面的讨论"——重要进展要及时写进 `FIELD_VALUE_REVIEW.md` / 本文档。
- 访问 9fzt 的权限用户已反复授权过, **不要再反复要求授权**。
- push 到 main 即时; VM 只在 worker tick 时 pull, 结果有延迟。并行提交 main 会 409 -> **脚本先、队列后、串行**。
- worker 幂等(结果存在即跳过) -> 重跑用新 id; 覆盖已存在路径要带 sha。
- stdout_tail 尾部截断 -> 关键输出放最后。
- **方法论元教训**: 线性 IC 判无价值 -> raw 分档均值判有价值 -> 严格检验(per-date 去均值/胜率/异常日)才见真相; 单表名次常无用, 价值常在跨表 level+Δ 交叉。
- 不要引用未核实的日期。
...
| 评审报告输出 | `projects/duanxianxia/reports/_audit/*.{json,md}` |

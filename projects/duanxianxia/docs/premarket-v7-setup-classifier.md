...
| `cashflow.stock.today` (T-1) | 覆盖全市场资金流入 top 150 (3day/5day/10day 仍为 100) | 候选股若 T-1 不在 top 150 → 该股**昨日资金不强势** |
...
- `cashflow.stock.{today,3day,5day,10day}` — 个股资金流 (today 前 **150** / 3day,5day,10day 各前 100)
...
- `cashflow.stock.*` (today=150, 3day/5day/10day 各 100)
...
```python
# 查询: T-1 cashflow.stock.today (覆盖全市场主力净流入 top 150) 是否包含 stock_code
...
if hit:
    ...
    if rank <= 50:
        ...
        stock_t1_label = 'hit_mid'   # 资金次主攻区 (50-150)
else:
    stock_t1_label = 'miss'  # 昨日不在前 150, 'quiet_yesterday'
```

**为什么 today 单独扩到 150**: hit_top 与 hit_mid 的区分需要中段资金信号. 若仍卡 100, 则 50-100 段被挤占, hit_mid 与 miss 边界模糊. 扩到 150 后 hit_top (1-50) / hit_mid (51-150) / miss (>150) 三档边界清晰, 对 Setup A/B 的资金验证更可靠. 3day/5day/10day 是趋势/连续性维度, 100 名足够区分 accumulating / distributing / neutral, 不需要扩.
...
for window in [3day, 5day, 10day]:
    if stock_code in cashflow.stock.{window}:  # 各窗口 top 100
...
| 盘后 17:20 | cashflow.stock.today | **150** | T-1 资金画像 (hit_top/hit_mid 分档) |
| 盘后 17:20 | cashflow.stock.{3day,5day,10day} | 各 100 | T-N 资金画像 (累积连续性) |
...
| cashflow.stock.today 上限 | 100 | **150** (区分 hit_top 1-50 / hit_mid 51-150) |
...
**前置依赖**: PR #9 (`feat/cashflow-today-limit-150`) 必须先合入并由 openclaw 部署, 否则 §3.2.2 的 hit_top/hit_mid 分档无数据基础 (top 100 数据集只能区分 1-50 vs 51-100, 无 50-150 段).

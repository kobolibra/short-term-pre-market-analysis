...
| `cashflow.stock.{today,3day,5day,10day}` (T-1) | 每个窗口覆盖全市场主力净流入 top 150 (由 PR #9 设定) | 候选股若在某窗口不在 top 150 → 该股**该窗口上资金不强势** |
...
- `cashflow.stock.{today,3day,5day,10day}` — 个股资金流 (4 个窗口各前 **150**, 由 PR #9 设定)
...
#### 3.2.2 个股 T-1 资金流命中 (today 窗口)

```python
# 查询: T-1 cashflow.stock.today (top 150) 是否包含 stock_code

if hit:
    rank = T-1.主力净流入排名  # 1-150
    if rank <= 50:
        stock_t1_label = 'hit_top'   # 资金主攻区 (top 50)
    else:
        stock_t1_label = 'hit_mid'   # 资金次主攻区 (51-150)
else:
    stock_t1_label = 'miss'  # 昨日不在前 150, 'quiet_yesterday'
```

**为什么 cashflow.stock.* 全 4 表都扩到 150**: 
- **对 today**: hit_top 与 hit_mid 的区分需要中段资金信号. 若仍卡 100, 则 50-100 段被挤占, hit_mid 与 miss 边界模糊. 扩到 150 后 hit_top (1-50) / hit_mid (51-150) / miss (>150) 三档边界清晰.
- **对 3day/5day/10day**: 见 §3.3.2 说明, 100->150 让 cashflow_continuity 能识别"资金从 top 100 掩出但尚未完全退场"的临界状态 (rank 100-150 区间), 这个状态在 100 上限下会被误判为 miss.
- **对反爬风险**: cashflow.stock.* 是单表分页 (非嵌套请求, 与 home.kaipan.plate.summary 子标签遍历不同), 每个窗口增加 1 页 (~50 行) 并不明显推高反爬画像, 每页间隔 1.8s 顺延总耗时仅 +7s.
...
for window in [3day, 5day, 10day]:
    if stock_code in cashflow.stock.{window}:  # 各窗口 top 150
        cashflow_{window}_pct = rank/150  # 0..1
    else:
...
# accumulating: 3day, 5day, 10day 三个窗口都命中且分位都 >= 0.5 (rank <= 75)
# distributing: 3day 命中且分位 < 0.3 (rank > 105), 但 5day/10day 分位 >= 0.5 (近期撤资)
# neutral: 其它命中情形
# miss: 三个窗口都未命中 (rank > 150)
```

**设计要点**: 100->150 后, distributing 状态能捆获一类之前被误分为 miss 的股票 ——「3day rank 在 100-150 区间」其实是「近期资金明显从主攻区掩出但尚未完全退场」, 在 v6 上下文会直接 NULL/miss 丢掉, v7 现在能保留这类信号。
...
stock_t1_label IN {'hit_top', 'hit_mid'}  # 昨日已强势 (top 150 内)
...
| 盘后 17:20 | cashflow.stock.* (4 个窗口) | **各 150** | T-1 + T-N 资金画像 |
...
| cashflow.stock.* 上限 (4 个窗口) | 100 | **150** (today 区分 hit_top 1-50 / hit_mid 51-150; 3day/5day/10day 获得 100-150 临界区间信号) |
...
**前置依赖**: PR #9 (`feat/cashflow-today-limit-150`, 名称保留但 scope 已扩到全 4 个窗口) 必须先合入并由 openclaw 部署。fetcher.py patch 已简化为 **单字符修改**: `CASHFLOW_DEFAULT_LIMIT = 100` → `CASHFLOW_DEFAULT_LIMIT = 150` (文件顶部常量区, 约第 28 行).
...
5. **stock_t1_label 阈值 rank<=50 切分点** 需要在双跑期间验证. 若 hit_top (1-50) 中 setup 命中率与 hit_mid (51-150) 无显著差异, 考虑降为 rank<=30.
6. **cashflow_continuity 阈值以 N=150 为底重新校准**: accumulating 的 0.5 分位 (rank<=75) / distributing 的 0.3 分位 (rank>105) 阈值需要在双跑期间验证. 在 N=100 时这些阈值是 rank<=50 / rank>70.

...
def build_premarket_analysis(report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    旧的 premarket_5table_v5 五表打分器是死代码，且权重与 v7.3 矛盾
    （v5 把竞价最后 1 秒抢筹当主信号，且完全不使用微买 weimai）。
    生产入口 premarket_v7_runner 早已 monkey-patch 成 v7.3；直接运行
    batch.py premarket 过去仍会走过时的 v5。现统一委托到 v7.3。
    函数内懒加载 import，避免与 v7 runner 的模块级循环 import。
    """

...
# -*- coding: utf-8 -*-
"""
duanxianxia_master_indicators.py -- Task 0113 (additive, read-only).
...
DATASETS = {
    ...
    "auction.jjlive.fengdan":    {"scope": STOCK, "canonical": False, "note": "竞价封单阶梯; 915/920/925=委买额(待确认 vs 成交)"},
    "home.ztpool":               {"scope": STOCK, "canonical": False, "note": "涨停晋级梯队; 状态 成/炸/败"},
    "review.fupan.plate":        {"scope": STOCK, "canonical": False, "note": "涨停复盘; FF/FLOAT/TOTAL 三口径黄金锚点表"},
    "review.ltgd.range":         {"scope": STOCK, "canonical": False, "note": "龙头高度区间涨幅; 5/10/20/50日"},
    "cashflow.stock.today":      {"scope": STOCK, "canonical": False, "note": "资金流向当日 n=50 (亿)"},
    "cashflow.stock.3day":       {"scope": STOCK, "canonical": False, "note": "资金流向 3日 n=150"},
    "cashflow.stock.5day":       {"scope": STOCK, "canonical": False, "note": "资金流向 5日 n=150"},
    "cashflow.stock.10day":      {"scope": STOCK, "canonical": False, "note": "资金流向 10日 n=150"},
    "rank.rocket":               {"scope": STOCK, "canonical": False, "note": "飞升榜; key=skyrocket_hour"},
    "rank.hot_stock_day":        {"scope": STOCK, "canonical": False, "note": "热度榜; BUG: 应读 hot_stock_hour"},
    "home.kaipan.plate.summary": {"scope": CONTEXT, "canonical": False, "note": "板块强度(非个股)"},
    "home.qxlive.top_metrics":   {"scope": CONTEXT, "canonical": False, "note": "大盘 12 指标(非个股)"},
    "review.daily.top_metrics":  {"scope": CONTEXT, "canonical": False, "note": "每日复盘顶部指标(非个股)"},
}
...
INDICATORS = {
    "free_float_mktcap": {
        ...
        "sources": [("auction.jjyd.net_amount", "raw[6] 亿->元"),
                    ...
                    ("review.fupan.plate", "实际流通 (EOD 锚点)")],
    ...
    "total_mktcap": {
        ...
        "sources": [("review.fupan.plate", "总市值")],
    },
    "auction_turnover": {
    ...
    # ---- index-only (来源表尚未登记 canonical; 索引已完整, 运行时暂无法拉) ----
    "limit_bid_amount_920": {
        "desc": "9:20 竞价委买额", "caliber": None, "unit": "元",
        "keys": ["amount_920", "limit_bid_amount_920"],
        "sources": [("auction.jjlive.fengdan", "amount_920 (委买/封单, 非成交)")],
    },
    "streak": {
        "desc": "连板数", "caliber": None, "unit": "板",
        "keys": ["streak", "连板"],
        "sources": [("review.fupan.plate", "连板"), ("home.ztpool", "分组名+状态")],
    },
    "seal_status": {
        "desc": "封板状态 成/炸/败", "caliber": None, "unit": "text",
        "keys": ["seal_status", "状态"],
        "sources": [("home.ztpool", "状态 token")],
    },
    "cashflow_main_net": {
        "desc": "资金流向主力净流入", "caliber": None, "unit": "亿",
        "keys": ["main_net_inflow", "主力净流入"],
        "sources": [("cashflow.stock.today", "主力净流入(亿)")],
    },
    "interval_change": {
        "desc": "区间涨幅(龙头高度)", "caliber": None, "unit": "%",
        "keys": ["interval_change", "区间涨幅"],
        "sources": [("review.ltgd.range", "区间涨幅")],
    },
}
...
if __name__ == "__main__":
    ...
    print("=== DUANXIANXIA MASTER INDICATOR INDEX + PANEL (Task 0113) ===")

"""v7.2 data loader: wrap v7.1 bundle and add T0 hotness datasets.

New optional T0 captures:
- rank.rocket        (飙升榜)
- rank.hot_stock_day (热度榜)

This loader is intentionally tolerant: if OpenClaw has not added these datasets
yet, v7.2 continues to run and hotness_score is treated as missing, not zero.
"""
...
def _latest_capture(project_root: Path, date_str: str, dataset_id: str, cutoff_hhmmss: Optional[str] = None) -> Tuple[List[Dict[str, Any]], Optional[str]]:
...
def load_premarket_v72_bundle(
    ...
    rocket, rocket_path = _latest_capture(project_root, date_str, DS_RANK_ROCKET, premarket_auction_cutoff)
    hotday, hotday_path = _latest_capture(project_root, date_str, DS_RANK_HOT_STOCK_DAY, premarket_auction_cutoff)
    kaipan_t0, kaipan_path = _latest_capture(project_root, date_str, DS_KAIPAN_PLATE_SUMMARY, premarket_auction_cutoff)

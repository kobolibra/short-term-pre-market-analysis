...
The important v7.2 change is that the report is no longer treated as one flat
"best stocks" list.  Premarket signals describe different trading actions:

- AUCTION_FOLLOW: strong auction continuation / follow-through candidate.
- THEME_CATCHUP: low-cost theme catch-up candidate; often weak auction but strong
  T0 plate match.
- LOW_OPEN_REVERSAL: low-open repair / reversal candidate.
- BOARD_WATCH: near-limit or locked-board candidate; good close_pct is common
  but post-auction excess_return can be small, so it must not compete directly
  with follow/catch-up/reversal candidates.
- CONFIRMATION_WATCH: useful but incomplete evidence.
- AVOID: fake strength, poor tradability, or hard risk.

This is deliberately not a one-day overfit.  Realized close_pct/excess_return is
used to inform the taxonomy, but production classification only uses information
available in the premarket report: auction_pct/latest_change_pct, auction
evidence, theme strength, hotness, order-book/fengdan state, and risk tags.
"""
...
DEFAULT_V72_ANCHORS = {
    ...
    "T0-REVERSAL": [
        "9:35 前是否快速收复竞价跌幅的 1/3",
        "9:45 前是否重新站上开盘价",
        "净额/成交额承接是否持续而不是一次性脉冲",
        "若 10:00 前仍弱于开盘价,降级为观察",
    ],
    "T0-DIVERGENCE": [
        "9:35 前是否有承接而非单边回落",
        "分歧放量后是否重新站上均价线",
...
def _theme_is_broad(theme_detail: Dict[str, Any], action_cfg: Dict[str, Any]) -> bool:
    ...
    if matched_plate in broad:
        return True
    for tag in theme_detail.get("matched_tags") or []:
        if str(tag) in broad:
            return True
    return False
...
def _low_cost_score(pct: Optional[float], lo: float = -1.5, hi: float = 3.5) -> float:
    if pct is None:
        return 40.0
    if lo <= pct <= hi:
        # Highest around small positive / near-flat auction; still allow mild
        # negative because theme catch-up often starts from flat/low-open.
        center = 1.0
...
def _classify_action(d: Dict[str, Any], action_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    risk_mult = float(_metric(d, "risk_multiplier", 1.0) or 1.0)
    trad_mult = float(_metric(d, "tradability_multiplier", 1.0) or 1.0)
    ...
    fengdan_status = str(a.get("fengdan_status") or "none")
    ...
    if entry_tag == "avoid" or auction_type == "FAKE_STRENGTH" or d.get("risk_penalty") == 0:
        ...
        return {
            ...
            "action_reason": "fake_strength_or_entry_avoid",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["AVOID"],
            "theme_is_broad": broad_theme,
        }

    # 2) Board / high-cost watch.  These can close strong, but the post-auction
    # excess return is structurally capped, so they should not compete with
    # follow/catch-up/reversal ideas.
    ...
    if entry_tag == "board_watch" or auction_type == "BOARD_LOCK_WATCH" or (pct is not None and pct >= board_pct):
        ...
        return {
            "action_type": "BOARD_WATCH",
            "action_confidence": _confidence_from_score(score, high=62, mid=42),
            "action_score": round(score, 2),
            "action_reason": "near_limit_or_locked_board_watch",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["BOARD_WATCH"],
            "theme_is_broad": broad_theme,
        }

    # 3) Low-open reversal.  Do not require theme strength: this is a repair
    # trade whose target is excess_return from a depressed auction price.
    reversal_min_auction = float(cfg.get("reversal_min_auction_strength", 25))
    if auction_type == "LOW_OPEN_REVERSAL" and pct is not None and pct < 0 and auction >= reversal_min_auction:
        score = _clamp(
            0.34 * auction
            + 0.22 * _pressure_score(net_pressure, float(cfg.get("net_pressure_full_ratio", 0.002)))
            + 0.18 * hot_score
            + 0.16 * min(100.0, source_evidence * 3.0)
            + 0.10 * liquidity
        )
        return {
            "action_type": "LOW_OPEN_REVERSAL",
            "action_confidence": _confidence_from_score(score, high=58, mid=38),
            "action_score": round(score, 2),
            "action_reason": "low_open_repair_with_premarket_support",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["LOW_OPEN_REVERSAL"],
            "theme_is_broad": broad_theme,
        }

    # 4) Auction follow-through.  This needs real auction evidence and a
    # reasonable cost basis.  It should not be triggered by theme alone.
    follow_min_auction = float(cfg.get("follow_min_auction_strength", 50))
    follow_min_source = float(cfg.get("follow_min_source_evidence", 18))
    follow_pct_lo = float(cfg.get("follow_pct_min", 2.0))
    follow_pct_hi = float(cfg.get("follow_pct_max", 7.5))
    has_follow_pct = pct is not None and follow_pct_lo <= pct <= follow_pct_hi
    has_follow_evidence = source_evidence >= follow_min_source or family_count >= int(cfg.get("follow_min_source_family_count", 2))
    if has_follow_pct and auction >= follow_min_auction and has_follow_evidence:
        score = _clamp(
            0.30 * auction
            + 0.23 * source_evidence
            + 0.17 * theme
            + 0.13 * money_score
            + 0.10 * resonance
            + 0.07 * liquidity
        ...
        return {
            "action_type": "AUCTION_FOLLOW",
            "action_confidence": _confidence_from_score(score, high=62, mid=45),
            "action_score": round(score, 2),
            "action_reason": "healthy_cost_auction_follow_through",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["AUCTION_FOLLOW"],
            "theme_is_broad": broad_theme,
        }

    # 5) Theme catch-up.  This is intentionally separate from auction-follow.
    # Broad themes are not killed: they are allowed only as low-cost catch-up or
    # with strong auction evidence.  This avoids one-day overfitting while using
    # the observed lesson that broad-but-real T0 themes can work.
    theme_min = float(cfg.get("theme_catchup_min_theme", 80))
    catchup_lo = float(cfg.get("theme_catchup_pct_min", -1.5))
    catchup_hi = float(cfg.get("theme_catchup_pct_max", 3.5))
    if theme >= theme_min and (pct is None or catchup_lo <= pct <= catchup_hi):
        low_cost = _low_cost_score(pct, catchup_lo, catchup_hi)
        score = _clamp(0.38 * theme + 0.24 * low_cost + 0.16 * auction + 0.12 * hot_score + 0.10 * liquidity)
        return {
            "action_type": "THEME_CATCHUP",
            "action_confidence": _confidence_from_score(score, high=62, mid=45),
            "action_score": round(score, 2),
            "action_reason": "low_cost_t0_theme_catchup",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["THEME_CATCHUP"],
            "theme_is_broad": broad_theme,
        }

    # 6) Useful but incomplete signal.
    if setup != "none" or confidence != "none":
        score = _clamp(0.32 * auction + 0.25 * theme + 0.15 * hot_score + 0.15 * source_evidence + 0.13 * liquidity)
        return {
            "action_type": "CONFIRMATION_WATCH",
            "action_confidence": _confidence_from_score(score, high=65, mid=40),
            "action_score": round(score, 2),
            "action_reason": "incomplete_or_single_factor_signal",
            "action_tags": action_tags,
            "action_priority": ACTION_PRIORITY["CONFIRMATION_WATCH"],
            "theme_is_broad": broad_theme,
        }
    ...
    return {
        ...
        "action_score": 0.0,
        "action_reason": "not_selected_but_kept_for_debug",
        "action_tags": action_tags,
        "action_priority": ACTION_PRIORITY["CONFIRMATION_WATCH"],
        "theme_is_broad": broad_theme,
    }
...
def _compact_decision(d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "action_confidence": d.get("action_confidence"),
        "action_score": d.get("action_score"),
        "action_reason": d.get("action_reason"),
        "action_tags": d.get("action_tags") or [],
        "theme_is_broad": d.get("theme_is_broad"),
        "final_score": d.get("final_score"),
        "today_signal_raw": d.get("today_signal_raw"),
        "auction_pct": auction_detail.get("latest_change_pct"),
        "auction_strength": d.get("auction_strength"),
        "theme_strength_t0": d.get("theme_strength_t0"),
        "hotness_score": d.get("hotness_score"),
        "auction_setup_type": d.get("auction_setup_type") or auction_detail.get("auction_setup_type"),
        "source_evidence_score": auction_detail.get("source_evidence_score"),
        "source_family_count": auction_detail.get("source_family_count"),
...
def _sort_action(decisions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return sorted(
        decisions or [],
        key=lambda x: (
            int(x.get("action_priority") or 999),
            -(float(x.get("action_score") or 0.0)),
...
def build_candidate_pools(decisions: List[Dict[str, Any]], pool_max: int = 15) -> Dict[str, List[Dict[str, Any]]]:
    ...
    pools: Dict[str, List[Dict[str, Any]]] = {
        "main_attack_pool": [],
        "theme_rotation_pool": [],  # kept for backward compatibility with v7.2 naming
        "theme_catchup_pool": [],
...
def build_intraday_anchors_v72(top_candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for d in top_candidates or []:
        ...
        action_extra = {
            "AUCTION_FOLLOW": [
                "9:35 前不能跌破开盘价",
                "10:00 前竞价强势来源至少保留一个: 抢筹/净额/量比",
            ],
            "THEME_CATCHUP": [
                "板块内是否继续扩散,不能只靠单票脉冲",
                "若 9:45 前仍弱于开盘价且板块退潮,降级",
            ],
            "LOW_OPEN_REVERSAL": [
                "优先看收复开盘价和均价线,不追弱反抽",
                "若净额承接消失或继续破低,放弃",
            ...
            "BOARD_WATCH": [
                "只按排板/回封逻辑观察,不和普通追涨票比较",
                "封单撤单明显时立即降级",
            ],
        }.get(str(action_type), [])
        compact.update({
            "setup_v71_compat": d.get("setup_v71_compat"),
            "anchors": (DEFAULT_V72_ANCHORS.get(key, DEFAULT_V72_ANCHORS["none"]) + action_extra),
...
def shape_v7_2_output(
    ...
    actionable = [
        d for d in action_ranked
        if d.get("action_type") in {"AUCTION_FOLLOW", "THEME_CATCHUP", "LOW_OPEN_REVERSAL", "BOARD_WATCH"}
    ]
    ...
    return {
        ...
        "meta": shaped_meta,
        "setup_stats": setup_stats_v72(enriched),
        "action_stats": action_stats_v72(enriched),
        "candidate_pools": pools,
        "top_candidates": top_actionable,
        "actionable_candidates": top_actionable,
        "legacy_top_candidates": legacy_top,
        "watch_tier": watch,
        "all_candidates_action_ranked": action_ranked,
        "all_candidates_debug": legacy_ranked,
        "intraday_anchors": build_intraday_anchors_v72(top_actionable[:20]),
    }
...
def _self_test() -> None:
    decisions = [
        {"code": "000001", "name": "A", "setup_v72": "T0-NEW", "setup_v71_compat": "D", "confidence": "high", "final_score": 88, "auction_strength": 80, "theme_strength_t0": 70, "auction_detail": {"latest_change_pct": 4.0, "source_evidence_score": 30, "source_family_count": 3, "money_intent_score": 80, "resonance_score": 80, "liquidity_score": 90, "qiangchou_primary_signal": "9:20-9:25"}, "theme_detail": {"matched_tags": ["算力"], "matched_plate": "算力"}},
        {"code": "000002", "name": "B", "setup_v72": "T0-GENERAL", "setup_v71_compat": "C2", "confidence": "low", "final_score": 50, "auction_strength": 10, "theme_strength_t0": 95, "auction_detail": {"latest_change_pct": 1.0, "source_evidence_score": 0, "liquidity_score": 70}, "theme_detail": {"matched_tags": ["一季报增长"], "matched_plate": "一季报增长"}},
        {"code": "000003", "name": "C", "setup_v72": "T0-REVERSAL", "setup_v71_compat": "REVERSAL", "confidence": "low", "final_score": 40, "auction_strength": 42, "hotness_score": 60, "auction_detail": {"auction_setup_type": "LOW_OPEN_REVERSAL", "latest_change_pct": -4.0, "source_evidence_score": 10, "net_pressure": 0.0015, "liquidity_score": 90}},
        ...
        {"code": "000005", "name": "E", "setup_v72": "T0-GENERAL", "setup_v71_compat": "C2", "confidence": "low", "final_score": 30, "auction_strength": 48, "auction_detail": {"latest_change_pct": 10.0, "orderbook_quality_score": 80, "liquidity_score": 90}, "entry_tag": "board_watch"},
    ]
    ...
    assert out["candidate_pools"]["theme_catchup_pool"], out
    assert out["candidate_pools"]["low_open_reversal_pool"], out
    assert out["candidate_pools"]["board_watch_pool"], out
    assert out["candidate_pools"]["avoid_or_risk_pool"], out
    assert out["legacy_top_candidates"], out
    print("output v7.2 action-pool _self_test passed")

...
Rollback note:
The elastic-first adaptive overlay over-promoted low-cost/20cm rows and degraded
2026-05-20 top30 performance.  This version restores the more balanced v7.3
structure: broad repair is a recall/ranking expansion, high-cost repair remains
watch-only, and expected-return ranking is display/ranking support rather than a
sector or board-style hard rule.

Rules remain production-safe:
- use only premarket-visible fields;
- realized returns stay review-only;
- do not hard-code any sector as the permanent answer;
- do not let 20cm/low-cost alone dominate the buy list.
"""
...
def apply() -> None:
    ...
    def diagnostics_next(rows: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for r in rows:
            ...
            if r.get("action_type") == "BROAD_REPAIR_MOMUM" and ex <= -3:
